...
class AanzPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('anz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `anz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM anz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO anz (date, title, link, content)"
